			...
			if(!FS::isAjaxCall()) {
				...
				if(!$sh || $sh == 1) {
					...
					if($data = FS::$dbMgr->Fetch($query)) {
						...
						$mtu = $this->devapi->getPortMtu($device);
						...
						$output .= "<tr><td>".$this->loc->s("admin-speed")."</td><td>";
                                                $sp = $this->devapi->getPortSpeed($device);
						if($sp > 0) {
						...
						$output .= "</td></tr>";
						$dup = $this->devapi->getPortDuplex($device);
						if($dup != -1) {
						...
						$trmode = $this->devapi->getSwitchportMode($device);

						$mabstate = $this->devapi->getSwitchportMABState($device);
						if($mabstate == 1)
							$trmode = 3;
						...
						switch($trmode) {
							case 1:
								...
								$nvlan = $this->devapi->getSwitchTrunkNativeVlan($device);
								$vllist = $this->devapi->getSwitchportTrunkVlans($device);
								break;
							case 2:
								$output .= $this->loc->s("Vlan");
								$nvlan = $this->devapi->getSwitchAccessVLAN($device);
								break;
							case 3:
								$output .= $this->loc->s("fail-vlan");
								$portoptlabel = $this->loc->s("MAB-opt");
								$nvlan = $this->devapi->getSwitchportAuthFailVLAN($device);
								break;
						}
						$output .= "</td><td id=\"vln\">";
						$voicevlanoutput = FS::$iMgr->selElmt($this->loc->s("None"),4096);
						$voicevlan = $this->devapi->getSwitchportVoiceVlan($device);
						$output .= FS::$iMgr->select("nvlan","");
						// Added none for VLAN fail
						if($trmode == 3)
							$output .= FS::$iMgr->selElmt($this->loc->s("None"),0,$nvlan == 0 ? true : false);

						$deadvlan = $this->devapi->getSwitchportAuthDeadVLAN($device);
						$deadvlanoutput = "";
						$norespvlan = $this->devapi->getSwitchportAuthNoRespVLAN($device);
						$norespvlanoutput = "";
						$trunkvlanoutput = "";
						$trunkall = true;
						$vlannb = 0;
						...
						*/
						...
						$mabeap = $this->devapi->getSwitchportMABType($device);
						$dot1xhostmode = $this->devapi->getSwitchportAuthHostMode($device);
						...
						*/
						...
						if(FS::$sessMgr->hasRight("mrule_switchmgmt_snmp_".$snmprw."_portmod_portsec") ||
							...
							$portsecen = $this->devapi->getPortSecEnable($device);
                	                                if($portsecen != -1) {
								...
								$output .= "<tr><td>".$this->loc->s("portsec-status")."</td><td>";
                        	                                $portsecstatus = $this->devapi->getPortSecStatus($device);
								...
								$psviolact = $this->devapi->getPortSecViolAct($device);
								...
								$psmaxmac = $this->devapi->getPortSecMaxMAC($device);
						...
						if(FS::$sessMgr->hasRight("mrule_switchmgmt_snmp_".$snmprw."_portmod_cdp") ||
							...
							$cdp = $this->devapi->getPortCDPEnable($device);
							if($cdp != NULL) {
						...
						if(FS::$sessMgr->hasRight("mrule_switchmgmt_snmp_".$snmprw."_portmod_dhcpsnooping") ||
							...
							$dhcpsntrust = $this->devapi->getPortDHCPSnoopingTrust($device);
							if($dhcpsntrust != NULL) {
							...
							$dhcpsnrate = $this->devapi->getPortDHCPSnoopingRate($device);
							if($dhcpsntrust != NULL) {
				...
				switch($act) {
					...
					case 3: // Desc fast edit
						...
						if(FS::$dbMgr->GetOneData("device_port","up","ip = '".$sw."' AND port = '".$port."'") != NULL) {
							if($this->devapi->setPortDesc($sw,$port,$desc) == 0) {
								echo $desc;
								if($save == "true")
									$this->devapi->writeMemory($sw);
					...
					case 9: // Switch Plug edit
						...
						$pid = $this->devapi->getPortId($sw,$port);
						if($pid == -1) {
						...
						$logvals = array();
						$idx = $this->devapi->getPortIndexes($sw,$pid);
						...
						if($duplex && FS::$secMgr->isNumeric($duplex)) {
							...
							if($idx != NULL) {
								$logvals["duplex"]["src"] = $this->devapi->getPortDuplex($sw);
						...
						if($speed && FS::$secMgr->isNumeric($speed)) {
							if($idx != NULL) {
								$logvals["speed"]["src"] = $this->devapi->getPortSpeed($sw);
								$this->devapi->setPortSpeedWithPid($sw,$idx[0].".".$idx[1],$speed);
								$logvals["speed"]["dst"] = $speed;
							}
						}
	
						$logvals["accessvlan"]["src"] = $this->devapi->getSwitchAccessVLAN($sw);
						$logvals["trunkencap"]["src"] = $this->devapi->getSwitchTrunkEncap($sw);
						$logvals["mode"]["src"] = $this->devapi->getSwitchportMode($sw);
						$logvals["trunkvlan"]["src"] = $this->devapi->getSwitchportTrunkVlans($sw);
						$logvals["nativevlan"]["src"] = $this->devapi->getSwitchTrunkNativeVlan($sw);
	
						// Mab & 802.1X
						$mabst = $this->devapi->getSwitchportMABState($sw);
						if($mabst != -1)
							$logvals["mabst"]["src"] = $mabst;
						$failvlan = $this->devapi->getSwitchportAuthFailVLAN($sw);
						if($failvlan != -1)
							$logvals["failvlan"]["src"] = $failvlan;
						$norespvlan = $this->devapi->getSwitchportAuthNoRespVLAN($sw);
						if($norespvlan != -1)
							$logvals["norespvlan"]["src"] = $norespvlan;
						$deadvlan = $this->devapi->getSwitchportAuthDeadVLAN($sw);
						if($deadvlan != -1)
							$logvals["deadvlan"]["src"] = $deadvlan;
						$controlmode = $this->devapi->getSwitchportControlMode($sw);
						if($controlmode != -1)
							$logvals["controlmode"]["src"] = $controlmode;
						$authhostmode = $this->devapi->getSwitchportAuthHostMode($sw);
						if($authhostmode != -1)
						...
						if($trunk == 1) {
							...
							$this->devapi->setSwitchAccessVLANWithPID($sw,$pid,1);
							$logvals["accessvlan"]["dst"] = 1;
							// mab disable
							if($mabst != -1) {
								$this->devapi->setSwitchportMABEnableWithPID($sw,$pid,2);
								$logvals["mabst"]["dst"] = 2;
							}
							if($failvlan != -1) {
								$this->devapi->setSwitchportAuthFailVLAN($sw,$pid,0);
								$logvals["failvlan"]["dst"] = 0;
							}
							if($norespvlan != -1) {
								$this->devapi->setSwitchportAuthNoRespVLAN($sw,$pid,0);
								$logvals["norespvlan"]["dst"] = 0;
							}
							if($deadvlan != -1) {
								$this->devapi->setSwitchportAuthDeadVLAN($sw,$pid,0);
								$logvals["deadvlan"]["dst"] = 0;
							}
							if($controlmode != -1) {
								$this->devapi->setSwitchportControlMode($sw,$pid,3);
								$logvals["controlmode"]["dst"] = 3;
							}
							// dot1x disable
							if($authhostmode != -1) {
								$this->devapi->setSwitchportAuthHostMode($sw,$pid,1);
								$logvals["authhostmode"]["dst"] = 1;
							}
	
							// set settings
							if($this->devapi->setSwitchTrunkEncapWithPID($sw,$pid,4) != 0) {
								FS::$iMgr->redir("mod=".$this->mid."&d=".$sw."&p=".$port."&err=2");
								return;
							}
							$logvals["trunkencap"]["dst"] = 4;
							if($this->devapi->setSwitchportModeWithPID($sw,$pid,$trunk) != 0) {
								FS::$iMgr->redir("mod=".$this->mid."&d=".$sw."&p=".$port."&err=2");
								return;
							}
							$logvals["mode"]["dst"] = $trunk;
							if(in_array("all",$vlanlist)) {
								if($this->devapi->setSwitchNoTrunkVlanWithPID($sw,$pid) != 0) {
									FS::$iMgr->redir("mod=".$this->mid."&d=".$sw."&p=".$port."&err=2");
									return;
								}
							}
							else {
								if($this->devapi->setSwitchTrunkVlanWithPID($sw,$pid,$vlanlist) != 0) {
									FS::$iMgr->redir("mod=".$this->mid."&d=".$sw."&p=".$port."&err=2");
									return;
								}
							}
							$logvals["trunkvlan"]["dst"] = $vlanlist;
							if($this->devapi->setSwitchTrunkNativeVlanWithPID($sw,$pid,$nvlan) != 0) {
								FS::$iMgr->redir("mod=".$this->mid."&d=".$sw."&p=".$port."&err=2");
								return;
							}
							$logvals["nativevlan"]["dst"] = $nvlan;
						} else if($trunk == 2) {
							$this->devapi->setSwitchTrunkNativeVlanWithPID($sw,$pid,1);
							$logvals["nativevlan"]["dst"] = 1;
							$this->devapi->setSwitchNoTrunkVlanWithPID($sw,$pid);
							...
							$logvals["mode"]["dst"] = $trunk;
							if($this->devapi->setSwitchTrunkEncapWithPID($sw,$pid,5) != 0) {
								if(FS::isAjaxCall())
							...
							if($this->devapi->setSwitchAccessVLANWithPID($sw,$pid,$nvlan) != 0) {
								if(FS::isAjaxCall())
							...
							$this->devapi->setSwitchNoTrunkVlanWithPID($sw,$pid);
							$logvals["trunkvlan"]["dst"] = "";
							$this->devapi->setSwitchportModeWithPID($sw,$pid,2);
							$logvals["mode"]["dst"] = 2;
							$this->devapi->setSwitchTrunkEncapWithPID($sw,$pid,5);
							$logvals["trunkencap"]["dst"] = 5;
							$this->devapi->setSwitchAccessVLANWithPID($sw,$pid,1);
							$logvals["accessvlan"]["dst"] = 1;
							...
							// enable mab
							if($mabst != -1) {
								$this->devapi->setSwitchportMABEnableWithPID($sw,$pid,1);
								$logvals["mabst"]["dst"] = 1;
							}
							$mabtype = $this->devapi->getSwitchportMABType($sw);
							if($mabtype != -1) {
								$logvals["mabtype"]["src"] = $mabtype;
								// set MAB to EAP or not
								$this->devapi->setSwitchMABTypeWithPID($sw,$pid,$mabeap == "on" ? 2 : 1);
								$logvals["mabtype"]["dst"] = ($mabeap == "on" ? 2 : 1);
							}
							if($failvlan != -1) {
								// enable authfail & noresp vlans
								$this->devapi->setSwitchportAuthFailVLAN($sw,$pid,$nvlan);
								$logvals["failvlan"]["dst"] = $nvlan;
							}
							if($norespvlan != -1) {
								$this->devapi->setSwitchportAuthNoRespVLAN($sw,$pid,$noresp);
								$logvals["norespvlan"]["dst"] = $noresp;
							}
							if($deadvlan != -1) {
								$this->devapi->setSwitchportAuthDeadVLAN($sw,$pid,$dead);
								$logvals["deadvlan"]["dst"] = $dead;
							}
							if($controlmode != -1) {
								// authentication port-control auto
								$this->devapi->setSwitchportControlMode($sw,$pid,2);
								$logvals["controlmode"]["dst"] = 2;
							}
							// Host Mode for Authentication
							if($authhostmode != -1) {
								$this->devapi->setSwitchportAuthHostMode($sw,$pid,$dot1xhostmode);
								$logvals["authhostmode"]["dst"] = $dot1xhostmode;
							}
						}
						$logvals["hostmode"]["src"] = $this->devapi->getPortState($sw);
						if($this->devapi->setPortStateWithPID($sw,$pid,($shut == "on" ? 2 : 1)) != 0) {
							if(FS::isAjaxCall())
						...
						$logvals["voicevlan"]["src"] = $this->devapi->getSwitchportVoiceVlan($sw);
						if($this->devapi->setSwitchportVoiceVlanWithPID($sw,$pid,$voicevlan) != 0) {
							if(FS::isAjaxCall())
						...
						if(FS::$sessMgr->hasRight("mrule_switchmgmt_snmp_".$snmprw."_portmod_cdp") ||
							FS::$sessMgr->hasRight("mrule_switchmgmt_ip_".$dip."_portmod_cdp")) {
							$cdpstate = $this->devapi->getPortCDPEnable($sw);
							if($cdpstate != NULL) {
						...
						if(FS::$sessMgr->hasRight("mrule_switchmgmt_snmp_".$snmprw."_portmod_dhcpsnooping") ||
							FS::$sessMgr->hasRight("mrule_switchmgmt_ip_".$dip."_portmod_dhcpsnooping")) {
							$dhcpsntruststate = $this->devapi->getPortDHCPSnoopingTrust($sw);
							...
							$dhcpsnrateorig = $this->devapi->getPortDHCPSnoopingRate($sw);
					...
					case 10: // replace vlan portlist
						...
						$plist = $this->devapi->getPortList($device,$vlan);
						$count = count($plist);
						if($count > 0) {
							echo "<ul>";
							for($i=0;$i<$count;$i++)
					...
					case 11: // Vlan replacement
						...
						$this->devapi->replaceVlan($device,$old,$new);
					...
					*/
					case 13:
						...
						echo $this->devapi->getCopyState($device,$saveid);
						return;
					case 14:
						$device = FS::$secMgr->checkAndSecuriseGetData("d");
						$saveid = FS::$secMgr->checkAndSecuriseGetData("saveid");
						...
						$err = $this->devapi->getCopyError($device,$saveid);
						switch($err) {
					...
					*/
					case 15:
						...
						echo $this->devapi->restoreStartupConfig($device);
						return;
					// Port monitoring
					case 16:
					...
					case 20: // Save all devices
						...
						$query = FS::$dbMgr->Select("device","name");
						while($data = FS::$dbMgr->Fetch($query)) {
							$this->devapi->writeMemory($data["name"]);
						}
					...
					case 21: // Backup all devices
						...
						while($data = FS::$dbMgr->Fetch($query)) {
							...
							while($data2 = FS::$dbMgr->Fetch($query2)) {
								if($data["type"] == 1)
									$copyId = $this->devapi->exportConfigToTFTP($data2["name"],$data["addr"],$data["path"]."conf-".$data2["name"]);
								else if($data["type"] == 2 || $data["type"] == 4 || $data["type"] == 5)
									$copyId = $this->devapi->exportConfigToAuthServer($data2["name"],$data["addr"],$data["type"],$data["path"]."conf-".$data2["name"],$data["login"],$data["pwd"]);
								
								sleep(1);
								$copyState = $this->devapi->getCopyState($data2["name"],$copyId);
								while($copyState == 2) {
									sleep(1);
									$copyState = $this->devapi->getCopyState($data2["name"],$copyId);
								}
								...
								if($copyState == 4) {
									$copyErr = $this->devapi->getCopyError($data2["name"],$copyId);
